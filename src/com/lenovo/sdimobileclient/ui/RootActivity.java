 ...
 * @Description: TODO
 */
public class RootActivity extends Activity implements IEncActivity, OnGetGeoCoderResultListener {
	...
	protected void showProgress(View progressLayout, boolean show) {
		...
		if (show) {
			...
			progressLayout.findViewById(android.R.id.progress).setVisibility(View.VISIBLE);
		} else {
	...
	protected void retryLoadData() {
		...
		notifyView();
		mHttpHelper.loadAgain();

		// mCacheManager.load(mCallbackId, mCacheParams, mCallback);
	}
